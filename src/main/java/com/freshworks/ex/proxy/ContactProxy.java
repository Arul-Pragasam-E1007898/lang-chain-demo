...
public class ContactProxy extends AbstractProxy {
    ...
    @Tool(name = "searchContact")
    public JsonNode searchContact(String email) throws IOException {
        logger.info("Fetching contact with Email: {}", email);
        Response response = restClient.get(REQUESTERS + "?email=" + email);
        if (!response.isSuccessful()) {
            logger.error("Failed to get contact. Status code: {}", response.code());
            throw new IOException("Failed to get contact: " + response.code());
        }
